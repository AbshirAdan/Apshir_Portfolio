...
const forgotPassword = async ({ email }) => {
  ...
  return {
    ...generic,
    resetToken: rawToken, // Kept for testing, but typically omitted in strict prod
  };
};

/** OAuth Universal Login Flow */
const oauthLogin = async (profile) => {
  const { provider, provider_id, email, full_name, avatar } = profile;
  
  // 1. Check if user already exists by provider and provider_id
  let user = await userRepo.findByProvider(provider, provider_id);

  if (!user) {
    // 2. Fallback check by email (if they registered locally then used OAuth)
    if (email) {
      user = await userRepo.findByEmail(email);
    }
    
    // 3. Create user if still doesn't exist
    if (!user) {
      user = await userRepo.createOAuthUser({
        full_name: full_name || 'Anonymous User',
        email: email || `${provider_id}@${provider}.oauth`,
        avatar,
        provider,
        provider_id,
        role: ROLES.USER
      ...
      // 4. If they exist but don't have provider linked, maybe we should link it?
      // (For this scope, we can just allow login to their existing account)
    }
  }
  ...
  const token = signToken({ id: user.id, email: user.email, role: user.role }, config.jwt.expiresIn);
