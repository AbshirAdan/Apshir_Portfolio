...
const ADMIN_USER_FIELDS = `
  id, full_name, email, role, avatar, phone, bio, status,
  created_at, updated_at, last_login
`;

const PUBLIC_USER_FIELDS = ADMIN_USER_FIELDS;
...
class UserRepository extends BaseRepository {
  ...
  async updateProfile(id, { full_name, email, phone, bio }) {
    const { rows } = await this.query(
      `UPDATE ${this.tableName}
       ...
       WHERE id = $5
       RETURNING ${PUBLIC_USER_FIELDS}`,
      [full_name, email?.toLowerCase(), phone, bio, id]
    );
    return rows[0] || null;
  }
